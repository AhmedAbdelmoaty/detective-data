...
import { EVIDENCE_ITEMS } from "@/data/case001";
                ...
                {data.invoices.map((inv: any) => (
                  <tr key={inv.id} className="border-b border-border/50">
                    ...
                    <td className="p-2 text-center">
                      {inv.hasReceipt ? <CheckCircle className="w-4 h-4 text-green-400 inline" /> : "✗"}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-xl">
            ...
            <div className="space-y-2 text-sm">
              <p className="text-foreground">
                <span className="text-muted-foreground">الكمية الأصلية:</span> {data.originalQty}
              </p>
              <p className="text-foreground">
                <span className="text-muted-foreground">الكمية المعدلة:</span>{" "}
                <span className="text-destructive font-bold">{data.modifiedQty}</span>
              </p>
              <p className="text-foreground">
                <span className="text-muted-foreground">المجموع الأصلي:</span> {data.originalTotal.toLocaleString()}
              </p>
              <p className="text-foreground">
                <span className="text-muted-foreground">المجموع المعدل:</span>{" "}
                <span className="text-destructive font-bold">{data.modifiedTotal.toLocaleString()}</span>
              </p>
                ...
                {data.projects.map((p: any) => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-border/50">
                      ...
                      <td
                        className={cn("p-2 font-mono", overBudget ? "text-destructive font-bold" : "text-foreground")}
                      >
                        {p.actualBought}
                      </td>
                    </tr>
      ...
      <motion.div className="absolute top-6 left-1/2 -translate-x-1/2 z-20">
        ...
        >
          ...
          <span className="mr-4 text-muted-foreground">
            أدلة: {state.collectedEvidence.length}/{EVIDENCE_ITEMS.length}
          </span>
        </div>
      </motion.div>
