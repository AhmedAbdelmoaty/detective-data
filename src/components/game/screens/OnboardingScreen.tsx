...
const slides = [
  {
    ...
    content: `أنت محقق بيانات تم استدعاؤك لحل أزمة في شركة عقارية.
...
${CASE_INFO.briefing}`,
  ...
  },
  {
    id: 2,
    title: "الشخصيات",
    content: `ستقابل 3 شخصيات رئيسية:

• ${CHARACTERS[0].name} - ${CHARACTERS[0].role} (قلق على الشركة)
• ${CHARACTERS[1].name} - ${CHARACTERS[1].role} (تدافع عن حملتها)
• ${CHARACTERS[2].name} - ${CHARACTERS[2].role} (محبط من العملاء)

استجوبهم بحكمة للوصول للحقيقة!`,
    icon: "👥",
  ...
  },
  {
    id: 3,
    title: "الغرف والأدلة",
    content: `ستتنقل بين 4 غرف:

👔 مكتب المدير التنفيذي - البداية والحوار مع محمد
📁 غرفة الأدلة - جميع المستندات والتقارير
📊 غرفة التحليل - الرسوم البيانية والجداول
👥 غرفة الاجتماعات - استجواب الموظفين

اجمع الأدلة، ثبّت المهم منها (حد أقصى 5)، وحلل البيانات!`,
    icon: "🗺️",
...
• كل سؤال وكل دليل يكلف وقتاً
• عندك 100 وحدة وقت فقط
• ثبّت الأدلة المهمة (حد أقصى 5)
• عندك 3 محاولات للتقرير النهائي

استخدم البيانات لتدعم استنتاجك قبل تقديم التقرير!`,
    icon: "⚠️",
